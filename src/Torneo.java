...
public class Torneo {
	...
	public boolean leerArchivo() {
		...
		try {
			...
			int cantConcursantes = Integer.parseInt(arch.nextLine()); // La primera linea del archivo nos dice cuantos
																		// participantes son
			for (int i = 0; i < cantConcursantes; i++) { // por cada participante que tome los lanzamientos
				...
				for (int tiro = 0; tiro < 3; tiro++) { // Todos los participantes tienen 3 lanzamientos
					String datos[] = (arch.nextLine()).split("	"); // Tomo la línea y la corto en 2 para parsear los
																	// datos
					Lanzamiento l = new Lanzamiento(Double.parseDouble(datos[0]), Double.parseDouble(datos[1]));// datos[0]
																												// =
																												// distancia
																												// |
																												// datos[1]
				...
				concursantes.add(new Concursante(actual, i + 1));// Uso el ctor de concursante con el array de
																	// lanzamientos
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean archivoSalida(String nombreArchivoSalida) {
		...
		try {
			...
			Juez j = new Juez(concursantes); // El constructor de juez calcula los ganadores
			for (int i : j.getGanadoresConsistencia()) {
				if (i != 0)
					pw.print(i + " ");
			}
			pw.print("\n");
			for (int i : j.getGanadoresDistancia()) {
				pw.print(i + " ");
			}
			pw.print("\n");
